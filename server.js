...
app.get('/api/namespaces/:resource', async (req, res) => {
    try {
        const resource = req.params.resource;
        const data = await SystemInfo.find().where('resource_name').equals(resource).select('-usage')
...
app.get('/api/:resource/:namespace/:limit', async (req, res) => {
    try {
        const resource = req.params.resource;
        ...
        return res.json(trimmedDocuments);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
});
